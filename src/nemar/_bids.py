"""BIDS-aware path parsing and semantic file selection."""
...
class BidsPath:
    ...
    def matches(self, query: BidsQuery) -> bool:
        """Return whether this path satisfies all query constraints."""
        for key, values in query.entities.items():
            if self.entities.get(key) not in values:
                return False
        if query.datatypes and self.datatype not in query.datatypes:
            return False
        if query.suffixes and self.suffix not in query.suffixes:
            return False
        if query.extensions and self.extension not in query.extensions:
            return False
        if query.scopes and self.scope not in query.scopes:
            return False
        if query.pipelines and self.pipeline not in query.pipelines:
            return False
        return True
...
class BidsQuery:
    ...
    def from_filters(
        ...
        *,
        ...
        """Build a query from common BIDS filters and generic entities."""
...
def _normalize_entity_mapping(
    ...
    if isinstance(entity, Mapping):
        return {
            _normalize_entity_key(key): _normalize_entity_values(key, value)
    ...
    for item in entity:
        ...
        out[_normalize_entity_key(key)] = _normalize_entity_values(key, value)
    return out
...
def _add_entity_from_token(entities: dict[str, str], token: str) -> None:
    ...
    if key and value:
        entities[_normalize_entity_key(key)] = value
